...
idxSpeech = detectSpeech(audioIn(:, 1), fs, 'Window', win, 'OverlapLength', overlap, 'MergeDistance', mergeDist) / fs; % Divide by fs for conversion to time
...
for i = 1:length(idxSpeech)
    ...
    featureSplices(i, :) = [tFE(boundLower) tFE(boundUpper) mean(feats(boundLower:boundUpper)) std(feats(boundLower:boundUpper))];
end
...
for p = 1:length(featureSplices)
    if p == length(featureSplices)
        break
    elseif abs(featureSplices(p + 1, 3) / featureSplices(p + 1, 4) - featureSplices(p, 3) / featureSplices(p, 4)) > detectThresh
...
for k = 1:length(detectedSplices)
    line([detectedSplices(k); detectedSplices(k)], [-0.3; 0.3], 'LineWidth', 2.5, 'LineStyle', '--', 'Color', '#A2142F');
end
...
for k = 1:length(detectedSplices)
    line([detectedSplices(k); detectedSplices(k)], [0.0779; 19.8], 'LineWidth', 2.5, 'LineStyle', '--', 'Color', '#FFFFFF');
end
